...
for fold, (train_ids, valid_ids) in enumerate(SKF.split(names_data,names_label)):
    ...
    if cfg.LOSS_FN =="MSE":
        ...
        if cfg.LABEL_SMOOTHING == False:
            ...
            train_loss_fn = LabelSmoothCrossEntropy(smoothing = cfg.LABEL_SMOOTHING_PARAM)
            val_loss_fn = LabelSmoothCrossEntropy(smoothing = cfg.LABEL_SMOOTHING_PARAM)
    elif cfg.LOSS_FN =="KL":
    ...
    for epoch in range(cfg.EPOCHS):
        ...
        # Train 
        for iteration, (str_imgs,top_imgs, sentinel_imgs, labels,dist_labels,cos_1) in enumerate(trainloader):
            ...
            if cfg.LOSS_FN == "MSE" or cfg.LOSS_FN == "MAE" :
                ...
                if cfg.COSINE_EMBEDDING == True:
                    loss_ce = train_loss_fn(pred, labels)
                    target_cos = torch.nn.functional.one_hot(labels.long(), 7)
                    cos_1 = cos_1.squeeze(-1)
                    loss_cos = cos_loss_fn(pred  ,target_cos, cos_1)
